...
function onCardInitialize() {
  ...
  DocCheckInProcess();
  if (!EdocsApi.getAttributeValue("DocExecutionTerm")?.value) EdocsApi.setAttributeValue({ code: "DocExecutionTerm", value: EdocsApi.getVacationPeriodEnd(new Date(), 30), text: null });
}
...
function calculateTotalSum2() {
  ...
  if (!totalSum2) {
    ...
    if (val && val > 0) EdocsApi.setAttributeValue({ code: "totalSum2", value: val });
  }
}
...
function onBeforeCardSave() {
  ...
  if (!EdocsApi.getAttributeValue("SumaAll")?.value) {
    if (EdocsApi.getCaseTaskDataByCode("Shtraf")?.state == "inProgress" || EdocsApi.getCaseTaskDataByCode("Shtraf1")?.state == "inProgress" || EdocsApi.getCaseTaskDataByCode("Shtraf2")?.state == "inProgress") throw "Не вірно введенні дані. Перевірте введену Задоволену суму по Таблицям";
  }
  if (!EdocsApi.getAttributeValue("DocExecutionTerm")?.value) EdocsApi.setAttributeValue({ code: "DocExecutionTerm", value: EdocsApi.getVacationPeriodEnd(new Date(), 30), text: null });
...
function setRegProp() {
  ...
  if (RegNumber && RegDate) {
    if (EdocsApi.getAttributeValue("ClaimNumber").value != RegNumber) EdocsApi.setAttributeValue({ code: "ClaimNumber", value: RegNumber, text: null });
    if (EdocsApi.getAttributeValue("ClaimDate").value != moment(RegDate).format("DD.MM.YYYY")) EdocsApi.setAttributeValue({ code: "ClaimDate", value: moment(RegDate).format("DD.MM.YYYY"), text: null });
  } else {
...
function setSumProp() {
  var totalSum2 = EdocsApi.getAttributeValue("totalSum2").value;
  if (totalSum2) {
    if (EdocsApi.getAttributeValue("SatisfiedAmount").value != totalSum2) EdocsApi.setAttributeValue({ code: "SatisfiedAmount", value: totalSum2, text: null });
  } else {
...
function setClaimTable(elName, setInvoice, setAmount) {
  ...
  if (el) {
    ...
    var buf = parentTable
      .filter((x) => {
        return x.find((y) => y.value == el);
      })
      ?.map((s) => s.find((n) => n.code == "Invoice_Claim_Amaunt2")?.value)[0];
    if (buf) {
...
function setSecondTableCall(elName, fromCall, toCall) {
  ...
  if (el) {
    ...
    var buf = parentTable
      ...
      ?.map((s) => s.find((n) => n.code == fromCall)?.value)[0];
    if (buf) {
...
function setSecondTableCallDictionary(elName, fromCall, toCall) {
  ...
  if (el) {
    ...
    var buf = parentTable
      ...
      ?.map((s) => s.find((n) => n.code == fromCall))[0];
    if (buf) {
  ...
  if (CostDocumentTable && CostDocumentTable.length > 0) {
    try {
      EdocsApi.setAttributeValue({
        code: "CostSumTotal",
        value: CostDocumentTable.flat()
          .filter((x) => x.code == "CostSum")
...
function RegistTask() {
  debugger;
  var stateTask = EdocsApi.getCaseTaskDataByCode("Regist")?.state;

  if (stateTask == "assigned" || stateTask == "inProgress" || stateTask == "delegated") {
    controlRequired("RequirementType");
    controlNotDisabled("RequirementType");
  } else if (stateTask == "completed") {
    controlRequired("RequirementType");
    controlDisabled("RequirementType");
  } else {
    controlNotRequired("RequirementType");
    controlNotDisabled("RequirementType");
  }
}
...
function onTaskExecuteRegist(routeStage) {
  if (routeStage.executionResult == "executed") {
    ...
    if (!EdocsApi.getAttributeValue("Regist").value) throw "Заповніть поле(я) «Тип вимог»";
  }
}
...
function createmethodData(eventType, typeDoc) {
  ...
  if (eventType == "2") {
    methodData.Attributes = RegistrationAtributs()
      .concat({ code: "DateSplat", value: EdocsApi.getAttributeValue("PaymentDate").value ? moment(EdocsApi.getAttributeValue("PaymentDate").value).format("YYYY-MM-DD") : null })
      .concat(FixationAtributs());
  }
...
function FixationAtributs() {
  return [{ code: "GuiltySubjects", type: "table", value: getClaimTable_Carrier() }];
}
...
function setRequirementTypeCodeForMask() {
  var RequirementType = EdocsApi.getAttributeValue("RequirementType")?.value;
  if (RequirementType) {
    var idDictionary = EdocsApi.getDictionaryData("RequirementType", RequirementType).find((x) => x)?.id;
    if (idDictionary) EdocsApi.setAttributeValue({ code: "RequirementTypeCodeForMask", value: EdocsApi.getDictionaryItemData("RequirementType", idDictionary)?.attributes.find((x) => x.code == "CodeRequirement").value });
  } else {
    EdocsApi.setAttributeValue({ code: "RequirementTypeCodeForMask", value: "" });
  }
}

function onChangeRequirementType() {
  setRequirementTypeCodeForMask();
}
